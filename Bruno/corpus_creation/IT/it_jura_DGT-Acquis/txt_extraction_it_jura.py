...
def extract_sentences_to_txt(path_to_file):
    ...
    decision_lib = data_string.split('Sentenza della Corte')
    for decision in decision_lib:
        if 500 < len(decision) < 50000:
            ...
            if text != '':
                print(f'Sentenza della Corte {paragraphs[0]}')
    ...
    with codecs.open("Decisions_IT.txt", 'w', encoding="utf-8") as f:
...
path_to_Parsable_XMLs = r'C:\Users\Arbeit\Desktop\Arbeit\Moralisierungen\Teil 8 Korpus Italienisch\Coli\DGT-Acquis\data.it.txt\data.it.txt'
