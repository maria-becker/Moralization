...
def test_for_interview(string, description):
    if ("interview" or "conversation") in string.lower()[0:500]:
        print(description)
        if string.count("?") > 0:
            return True
    return False
...
def print_moral_sentences(corpus_file_name, sheet_name):
    ...
    with open(corpus_file_name, 'r', encoding='utf-8', errors="ignore") as f:
