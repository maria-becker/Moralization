...
def add_excel_sheets(list_of_genres, list_of_sheets, role):

    # Delete old data
    with pd.ExcelWriter(f'all_texts-{role}.xlsx') as writer:
        df = pd.DataFrame()
    ...
    for sheet in list_of_sheets:
        ...
        for name in list_of_genres:
            ...
            # Open the Excel file
            excel_file = pd.ExcelFile(f"{name}-{role}.xlsx")
        ...
        with pd.ExcelWriter(f'all_texts-{role}.xlsx', mode="a", engine="openpyxl") as writer:
...
add_excel_sheets(all_genres, all_sheets, "Forderer")
